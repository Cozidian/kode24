...
2. Write 3–5 failing tests that cover the most important behaviours from backend to UI. Follow the project's test conventions exactly:
   ...
   - For `Combat.tick/4` tests, pattern-match on the tagged result tuple: `{:continue | :monster_dead | :player_dead, player, monster, log}`
   - For LiveView tests, use `Phoenix.LiveViewTest`: navigate to `~p"/"`, click `"button[phx-click=start_game]"` to enter fighting phase, then assert with `has_element?/2`
